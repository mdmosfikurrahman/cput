...
public class CustomerProfilePanel extends JPanel {
    ...
    public CustomerProfilePanel(Customer customer, CustomerService customerService) {
    ...
    private void initComponents() {
        ...
        loadProfileImage();
        mainPanel.add(profileImageLabel, gbc);

        nameField = createStyledTextField(customer.getName());
        emailField = createStyledTextField(customer.getEmail());
        phoneField = createStyledTextField(customer.getPhone());
    ...
    private void loadProfileImage() {
        try {
            // Assuming the profile picture is named "customerProfile.jpeg" and is in the project's root directory
            File imageFile = new File("customerProfile.jpeg");
            if (imageFile.exists()) {
                ...
                profileImageLabel.setIcon(new ImageIcon(circleBuffer));
            } else {
    ...
    private void updateProfile() {
        ...
        try {
            ...
            customerRQ.setName(newName);
            customerRQ.setEmail(newEmail);
            customerRQ.setPhone(newPhone);
            customerRQ.setNic(customer.getNic());
            if (!newPassword.isEmpty()) {
                customerRQ.setPassword(newPassword); // In real-world, hash this password
            }
